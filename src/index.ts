...
class YandexDiskClient {
  ...
  async logIn(){
    console.log(`Logging in using login ${this.login}, password ${this.password} and phone ${this.phoneNumber}`);

    const preauthResult = await YaAuth.YadPreAuthRequest(this);
    console.log('preauth result: ', preauthResult);

    const loginRequest = await YaAuth.YadAuthLoginRequest(this, preauthResult.csrf, preauthResult.uuid);

    const passwordRequest = await YaAuth.YadAuthPasswordRequest(this, preauthResult.csrf, loginRequest.track_id);

    const accountsRequest = await YaAuth.YadAuthAccountsRequest(this, preauthResult.csrf);
