...
export default {
  async getFolderInfo(client: YandexDiskClient, path: string) {
    ...
    const result = await client.httpClient.post('https://disk.yandex.ru/models/?_m=resources',
    {
      form: {
  ...
  async getFileUrl(client: YandexDiskClient, path: string) {
    ...
    const result = await client.httpClient.post('https://disk.yandex.ru/models/?_m=do-get-resource-url',
    {
      form: {
        idClient: client.idClient,
        sk: client.skToken,
  ...
  async uploadFile(client: YandexDiskClient, path: string, buffer: Buffer){
    ...
    if (uploadData?.data?.error){
      ...
      throw new Error('Failed to upload file:' + JSON.stringify(uploadData.data.error));
    }
    ...
    if (uploadData?.data?.upload_url){
      ...
      // upload file to url
      const putResult = await client.httpClient.put(uploadData?.data?.upload_url, {
        headers: { 
          Referer: 'https://disk.yandex.ru/client/disk?source=landing2_signin_ru',
      ...
      const verifyResult = await client.httpClient.post('https://disk.yandex.ru/models/?_m=do-status-operation', {
        headers: { 
          Referer: 'https://disk.yandex.ru/client/disk?source=landing2_signin_ru',
          // 'X-Disk-Uploader-Wait-Complete-Upload': false
        },
  ...
  async createFolder(client: YandexDiskClient, path: string){
    ...
    const createFolderResult = await client.httpClient.post('https://disk.yandex.ru/models/?_m=do-resource-create-folder', {
      headers: { 
        Referer: 'https://disk.yandex.ru/client/disk?source=landing2_signin_ru',
        // 'X-Disk-Uploader-Wait-Complete-Upload': false
      },
